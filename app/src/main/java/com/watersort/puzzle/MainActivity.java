...
public class MainActivity extends AppCompatActivity implements GameManager.GameListener {
    ...
    // ── ИСПРАВЛЕНИЕ: используем tag вместо лямбды в цикле ─────────────
    private void addTubeView(LinearLayout parent, Tube tube, int index) {
        TubeView tv = new TubeView(this);
        tv.setTube(tube);
        tv.setTag(index); // сохраняем индекс в tag
        tv.setOnTubeClickListener(new TubeView.OnTubeClickListener() {
            @Override
            public void onTubeClick(TubeView tubeView) {
                int idx = (int) tubeView.getTag();
